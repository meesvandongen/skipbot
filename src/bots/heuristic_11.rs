...
impl Heuristic11Bot {
    ...
    fn can_play_stock(state: &GameStateView, legal_actions: &[Action]) -> Option<Action> {
        ...
        if let Card::SkipBo = stock {
            ...
            {
                let action = Action::Play { source: CardSource::Stock, build_pile: best_idx };
        ...
        #[derive(Clone, Copy)]
        enum SourceKind { Hand(usize), Discard(usize) }
        ...
        for (idx, card) in state.hand.iter().copied().enumerate() {
            match card { Card::Number(v) => by_value[v as usize].push(SourceKind::Hand(idx)), Card::SkipBo => skipbo_hands.push(idx) }
        }
        for (d_idx, top) in Self::self_player(state).discard_tops.iter().copied().enumerate() {
            if let Some(card) = top {
                match card { Card::Number(v) => by_value[v as usize].push(SourceKind::Discard(d_idx)), Card::SkipBo => skipbo_discards.push(d_idx) }
            }
        }
        ...
        for (pile_idx, pile) in state.build_piles.iter().enumerate() {
            ...
            for need in required.iter().copied() {
                ...
                for &src in &by_value[need as usize] { if let SourceKind::Discard(d) = src { if !used_discard[d] { picked = Some(src); break; } } }
                if picked.is_none() { for &src in &by_value[need as usize] { if let SourceKind::Hand(h) = src { if !used_hand[h] { picked = Some(src); break; } } } }
                if picked.is_none() { if let Some(d) = skipbo_discards_left.iter().copied().find(|&d| !used_discard[d]) { picked = Some(SourceKind::Discard(d)); } else if let Some(h) = skipbo_hands_left.iter().copied().find(|&h| !used_hand[h]) { picked = Some(SourceKind::Hand(h)); } }
                let Some(src) = picked else { possible = false; break; };
                match src {
                    SourceKind::Discard(d) => { used_discard[d] = true; actions.push(Action::Play { source: CardSource::Discard(d), build_pile: pile_idx }); },
                    SourceKind::Hand(h) => { used_hand[h] = true; actions.push(Action::Play { source: CardSource::Hand(h), build_pile: pile_idx }); },
                }
                _sim_next = if _sim_next == MAX_CARD_VALUE { 1 } else { _sim_next + 1 };
            }
            if !possible { continue; }
            match &mut best_plan { None => best_plan = Some((pile_idx, actions)), Some((_best_idx, best_actions)) => { if actions.len() < best_actions.len() { *best_actions = actions; *_best_idx = pile_idx; } } }
        }
        if let Some((pile_idx, actions)) = best_plan {
            if actions.is_empty() {
                let action = Action::Play { source: CardSource::Stock, build_pile: pile_idx };
                if legal_actions.contains(&action) { return Some(action); }
            } else {
                let first = actions[0].clone();
                if legal_actions.contains(&first) { return Some(first); }
            }
        }
        None
    }
    ...
    fn can_play_all_hand(state: &GameStateView, legal_actions: &[Action]) -> Option<Action> {
        ...
        for (i, pile) in state.build_piles.iter().enumerate() { piles_next[i] = pile.next_value; }
        ...
        let is_first_action_legal = |hand_idx: usize, pile_idx: usize| -> bool {
            legal_actions.contains(&Action::Play { source: CardSource::Hand(hand_idx), build_pile: pile_idx })
        };
                ...
                match card {
                    Card::SkipBo => {
                        for pi in 0..piles.len() {
                            ...
                            if path.is_empty() && !first_legal(hi, pi) { continue; }
                            let old = piles[pi];
                            piles[pi] = inc(piles[pi]);
                            used[hi] = true;
                            path.push((hi, pi));
                            if dfs(depth + 1, piles, used, hand, path, first_legal) { return true; }
                            path.pop();
                            used[hi] = false;
                            piles[pi] = old;
                        }
        ...
        if dfs(0, &mut piles_next, &mut used, &state.hand, &mut path, &is_first_action_legal) {
    ...
    fn played_card_value(state: &GameStateView, action: &Action) -> Option<u8> {
        if let Action::Play { source, .. } = action {
            match *source {
                CardSource::Hand(i) => match state.hand.get(i).copied()? { Card::Number(v) => Some(v), Card::SkipBo => None },
                CardSource::Discard(d) => match Self::self_player(state).discard_tops[d]? { Card::Number(v) => Some(v), Card::SkipBo => None },
                CardSource::Stock => match Self::self_player(state).stock_top? { Card::Number(v) => Some(v), Card::SkipBo => None },
            }
        } else { None }
    }
    ...
    fn breaks_pair_duplication(state: &GameStateView, action: &Action) -> bool {
        let Action::Play { build_pile, .. } = action else { return false }; // non-play actions irrelevant
        let old_next = state.build_piles[*build_pile].next_value;
        let mut count = 0usize;
        for pile in state.build_piles.iter() { if pile.next_value == old_next { count += 1; } }
        count == 2
    }
}

impl Default for Heuristic11Bot { fn default() -> Self { Self::new() } }

impl Bot for Heuristic11Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(!legal_actions.is_empty(), "heuristic 11 bot requires at least one legal action");

        // 1) Stock-first plan (same as heuristic 10).
        if let Some(action) = Self::can_play_stock(state, legal_actions) { return action; }

        // 2) Hand-empty plan: if we can play ALL current hand cards in sequence, do it.
        if let Some(action) = Self::can_play_all_hand(state, legal_actions) { return action; }

        // 3) Number play selection with duplication preservation (same as heuristic 10).
        let stock_value = match Self::self_player(state).stock_top { Some(Card::Number(v)) => v, Some(Card::SkipBo) | None => 1 };
        let mut best_play: Option<(u8, usize, Action)> = None; // (card_value, pile_len, action)
        for action in legal_actions.iter() {
            if !matches!(action, Action::Play { .. }) { continue; }
            if Self::breaks_pair_duplication(state, action) { continue; }
            if let Some(v) = Self::played_card_value(state, action) { if v >= 6 && v > stock_value { let pile_len = match action { Action::Play { build_pile, .. } => state.build_piles[*build_pile].cards.len(), _ => 0 }; let better = match &best_play { None => true, Some((best_v, best_len, _)) => v > *best_v || (v == *best_v && pile_len > *best_len) }; if better { best_play = Some((v, pile_len, action.clone())); } } }
        }
        if let Some((_, _, action)) = best_play { return action; }

        // 4) Discard scoring.
        let mut best: Option<(i32, Action)> = None;
        for action in legal_actions.iter() {
            if let Action::Discard { hand_index, discard_pile } = *action { let score = Self::score_discard(state, hand_index, discard_pile); if best.as_ref().map(|(s, _)| score > *s).unwrap_or(true) { best = Some((score, action.clone())); } }
        }
        if let Some((_, action)) = best { return action; }

        // 5) Fallback: prefer a non-blocked play or any non-play; also avoid blocked plays.
        if let Some(action) = legal_actions.iter().find(|a| !matches!(a, Action::Play { .. }) || !Self::should_block_play(state, a)) { return action.clone(); }

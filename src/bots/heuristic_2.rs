...
/// Heuristic 2 bot
/// Strategy:
/// 1) Try to determine if it's possible to play the stock card this turn using only
///    currently available cards (hand + tops of discard piles) and build piles.
///    If yes, compute the shortest sequence of plays needed and execute the first action now.
/// 2) Otherwise, choose a Discard using the same logic as Heuristic 1.
pub struct Heuristic2Bot;
...
impl Heuristic2Bot {
    ...
    /// Same discard scoring as in heuristic_1.
    fn card_priority(card: Card) -> i32 {
        match card {
            Card::Number(v) => v as i32,
...
impl Default for Heuristic2Bot {
    fn default() -> Self {
        Self::new()
    }
}

impl Bot for Heuristic2Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(
            !legal_actions.is_empty(),
            "heuristic 2 bot requires at least one legal action"
        );
        ...
        // 2) Otherwise, choose a discard using Heuristic 1's logic.
        ...
        // 3) Fallback: if no discard is available, just pick the first legal action.

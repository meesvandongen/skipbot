...
/// Heuristic 7 bot
/// Strategy:
/// 1) Try stock-first planning (as in heuristic 2/3/4/5/6): if the stock can be played this turn
...
///    - its value is greater than or equal to 7.
...
impl Default for Heuristic7Bot {
    fn default() -> Self {
        Self::new()
    }
}

impl Bot for Heuristic7Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(
            !legal_actions.is_empty(),
            "heuristic 7 bot requires at least one legal action"
        );
        ...
        // 2) Otherwise, play only number cards where value > stock value AND value >= 7.

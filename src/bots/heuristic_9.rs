...
/// Heuristic 9 bot (based on Heuristic 8)
/// Difference from Heuristic 8:
/// - Do NOT block plays (including stock-first planning) that would set up the next
///   player's stock; ONLY apply that block rule during the final fallback selection
///   of an action when nothing else qualified.
pub struct Heuristic9Bot;
...
impl Heuristic9Bot {
    ...
    /// Note: Unlike Heuristic 8, we do NOT block potential setup-for-next-player in this phase.
...
impl Default for Heuristic9Bot {
    fn default() -> Self {
        Self::new()
    }
}

impl Bot for Heuristic9Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(
            !legal_actions.is_empty(),
            "heuristic 9 bot requires at least one legal action"
        );

        // 1) Try stock-first plan (no blocking in this phase).
        if let Some(action) = Self::can_play_stock(state, legal_actions) {
            return action;
        }

        // 2) Otherwise, play only number-like values where value > stock value AND value >= 6.
        //    (Skip-Bo counts as the pile's next_value as in Heuristic 8.)
        ...
        // 4) Fallback: prefer a non-blocked action, else take the first legal action.

...
/// Heuristic 3 bot
/// Strategy:
/// 1) Same stock-first planning as Heuristic 2: try to determine if it's possible to play the
...
///    first action now.
/// 2) Otherwise, choose a Discard similar to Heuristic 2 but IGNORING card priority.
pub struct Heuristic3Bot;
...
impl Heuristic3Bot {
    ...
    /// Discard scoring: identical to heuristic_2 except we IGNORE card priority.
...
impl Default for Heuristic3Bot {
    fn default() -> Self {
        Self::new()
    }
}

impl Bot for Heuristic3Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(
            !legal_actions.is_empty(),
            "heuristic 3 bot requires at least one legal action"
        );
        ...
        // 2) Otherwise, choose a discard using the modified logic (no card priority).

...
/// Heuristic 5 bot
/// Strategy:
/// 1) Try stock-first planning (as in heuristic 2/3/4): if the stock can be played this turn
...
///    - its value is greater than or equal to 6.
...
impl Default for Heuristic5Bot {
    fn default() -> Self {
        Self::new()
    }
}

impl Bot for Heuristic5Bot {
    fn select_action(&mut self, state: &GameStateView, legal_actions: &[Action]) -> Action {
        assert!(
            !legal_actions.is_empty(),
            "heuristic 5 bot requires at least one legal action"
        );
